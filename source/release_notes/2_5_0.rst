...
- Fix 500 status code then try to update subscrition if one of subscribed triggers was removed `#271 <https://github.com/moira-alert/moira/pull/271>`_.
- Small optimizations of metric parser in filter `#267 <https://github.com/moira-alert/moira/pull/267>`_.
- Add meaningful title to all moira web pages `#177 <https://github.com/moira-alert/moira/issues/177>`_.
- Properly encode parameters that passed in web to api requests `#174 <https://github.com/moira-alert/moira/issues/174>`_.
...
- Fix external loader on non-existing trigger page in mobile version of web `#168 <https://github.com/moira-alert/moira/issues/168>`_.
- Remove cancel button and restyle delete button in subscription modal `#221 <https://github.com/moira-alert/moira/issues/221>`_.
- Correctly handle metric that received in filter with windows line breaks (/r/n) `#268 <https://github.com/moira-alert/moira/pull/268>`_.
...
- No more allow to use ``ERROR_VALUE`` and ``WARN_VALUE`` in expression as varables. Your old triggers with this variables will still work, but you can not update this triggers until you delete ``ERROR_VALUE`` and ``WARN_VALUE`` variables from expression `#172 <https://github.com/moira-alert/moira/issues/172>`_.
...
- Add description for stop sending notifications log message, then self state monitor is turned on `#323 <https://github.com/moira-alert/moira/pull/323>`_.
...
- Fix sending a message "This metric changed its state..." if state does not change during maintenance interval `#328 <https://github.com/moira-alert/moira/issues/328>`_.
- Add Webhook sender `#123 <https://github.com/moira-alert/moira/issues/123>`_. For more info see documantation_.
- Add to script sender new variables. Variable ${trigger_name} is deprecated, removed from documentation and will be removed in future versions of moira `#228 <https://github.com/moira-alert/moira/issues/228>`_. For more inforamation about new variables and script configuration see documantation_.
- Fix update event timestamp on maintenance metric in changed suppressed state on every trigger check `#332 <https://github.com/moira-alert/moira/issues/332>`_.