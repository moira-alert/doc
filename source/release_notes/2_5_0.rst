...
- Изменили контракт контактов в конфиге, чтобы исправить проблему захардкоженных типов контактов в форме добавления контакта на странице Уведомлений в веб-морде. Теперь список допустимых к добавлению контактов строится на основе конфига (как должно было быть изначально). А значение, введённое пользователем, валидируется по переданному регулярному выражению. Поле title было переименовано в правильное placeholder. А подпись у контакта строится не на type, а передаётся в label. Что делает настройку ещё гибче.
